...
- [x] Create directory `apps/mfe-catalog/`
- [x] Create `apps/mfe-catalog/package.json` with:
  - `"type": "module"`
...
- [x] Create `apps/mfe-catalog/tsconfig.json` (target ES2020, `moduleResolution: bundler`, `jsx: react-jsx`, strict mode)
- [x] Create `apps/mfe-catalog/tsconfig.node.json` for Vite config compilation
- [x] Create `apps/mfe-catalog/index.html` with `<div id="root">` and `<script type="module" src="/src/main.tsx">`

## 4.2 Vite configuration

- [x] Create `apps/mfe-catalog/vite.config.ts`:
...
- [x] Create `apps/mfe-catalog/vitest.config.ts`:
  - `environment: 'jsdom'`
  - `globals: true`
...
- [x] Create `apps/mfe-catalog/tailwind.config.ts` extending the default theme with custom colors from `docs/search_page_design.json`:
...
- [x] Create PostCSS config (`apps/mfe-catalog/postcss.config.cjs`) with `tailwindcss` and `autoprefixer` plugins
- [x] Create `apps/mfe-catalog/src/index.css` with `@tailwind base/components/utilities` directives
- [x] Use Tailwind utilities for component styling (no component-scoped `.css` files under `src/components`)
- [x] Use semantic Tailwind tokens (`bg-page-bg`, `text-primary-text`, `rounded-card`) instead of raw hex values in JSX

## 4.5 Types

- [x] Create `apps/mfe-catalog/src/types/product.ts` with interfaces:
...
- [x] Create `apps/mfe-catalog/src/api/graphqlClient.ts`:
  - `GraphQLClient` pointed at `import.meta.env.VITE_GRAPHQL_URL ?? 'http://localhost:8080/graphql'`
- [x] Create `apps/mfe-catalog/src/api/queries/products.gql.ts`:
...
- [x] Create `apps/mfe-catalog/src/api/queries/product.gql.ts`:
  - `PRODUCT_QUERY` — GQL query for `product(id)` returning all fields
  - Export `ProductQueryResult` TypeScript interface
- [x] Create `apps/mfe-catalog/.env` with `VITE_GRAPHQL_URL=http://localhost:8080/graphql`

## 4.7 TanStack Store — filter state

- [x] Create `apps/mfe-catalog/src/store/filterStore.ts`:
...
- [x] Create `apps/mfe-catalog/src/hooks/useProducts.ts`:
...
- [x] Create `apps/mfe-catalog/src/hooks/useProduct.ts`:
  - `useProduct(id)` — query key `['product', id]`, calls `graphqlClient.request(PRODUCT_QUERY, { id })`, `staleTime: 300_000`, `enabled: Boolean(id)`

## 4.9 Entry points

- [x] Create `apps/mfe-catalog/src/main.tsx` — single line: `import('./bootstrap')`
- [x] Create `apps/mfe-catalog/src/bootstrap.tsx`:
...
- [x] Create `apps/mfe-catalog/src/App.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/SearchBar/SearchBar.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/FilterBar/FilterDropdown.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/FilterBar/FilterPill.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/FilterBar/FilterBar.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/StoreRow/StoreCard.tsx`:
  - Props: `store: Store`
...
- [x] Create `apps/mfe-catalog/src/components/StoreRow/StoreRow.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/ProductGrid/ProductCardSkeleton.tsx`:
  - `animate-pulse` gray blocks mimicking image + text layout
  - `aria-busy="true"`, `aria-label="Loading product"`
- [x] Create `apps/mfe-catalog/src/components/ui/StarRating.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/ui/WishlistButton.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/ProductGrid/ProductCard.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/ProductGrid/ProductGrid.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/CatalogPage.tsx`:
...
- [x] Create `apps/mfe-catalog/src/components/ProductDetail/ProductDetailPage.tsx`:
...
- [x] Create `apps/mfe-catalog/src/test/mswServer.ts`:
...
### FilterBar tests
- [x] Create `apps/mfe-catalog/src/__tests__/components/FilterBar.test.tsx`:
  - [x] **AC-FB-1** Renders Category and Sort by pills
  - [x] **AC-FB-2** Clicking Category pill opens a `role="listbox"` dropdown
  - [x] **AC-FB-3** Selecting "Electronics" from the dropdown sets category in store and closes dropdown
  - [x] **AC-FB-4** Active filter pill has visually distinct (link-coloured) styling

### ProductCard tests
- [x] Create `apps/mfe-catalog/src/__tests__/components/ProductCard.test.tsx`:
  - [x] **AC-PC-1** Renders product name and formatted price
  - [x] **AC-PC-2** Clicking "Add to cart" dispatches a `cart:add-item` CustomEvent with `productId, productName, price, quantity: 1`
  - [x] **AC-PC-3** Wishlist button has `aria-pressed="false"` by default; toggles to `aria-pressed="true"` on click
  - [x] **AC-PC-4** Wishlist button click does not trigger Link navigation

### ProductGrid tests
- [x] Create `apps/mfe-catalog/src/__tests__/components/ProductGrid.test.tsx`:
  - [x] **AC-PG-1** Shows 10 elements with `aria-label="Loading product"` when `isLoading=true`
  - [x] **AC-PG-2** Renders a product card for each item in `products` array
  - [x] **AC-PG-3** Pagination controls are absent when `totalPages = 1`
  - [x] **AC-PG-4** Previous button is disabled when `currentPage = 0`
  - [x] **AC-PG-5** Clicking Next calls `onPageChange(1)` when on page 0

### SearchBar tests
- [x] Create `apps/mfe-catalog/src/__tests__/components/SearchBar.test.tsx`:
  - [x] **AC-SB-1** Renders an `<input type="search">` with `aria-label="Search products"`
  - [x] **AC-SB-2** `onChange` is NOT called synchronously after typing a single character
  - [x] **AC-SB-3** `onChange` is called once after the debounce timer fires (use `vi.useFakeTimers`)
  - [x] **AC-SB-4** Rapid successive keystrokes produce only one `onChange` call

### StoreRow tests
- [x] Create `apps/mfe-catalog/src/__tests__/components/StoreRow.test.tsx`:
  - [x] **AC-SR-1** All 4 mock store names are present in the rendered output
  - [x] **AC-SR-2** Each store card contains a "Visit shop" link
  - [x] **AC-SR-3** Scroll button has `aria-label="Scroll stores right"` and is focusable

## 4.18 Hook tests

### useProducts tests
- [x] Create `apps/mfe-catalog/src/__tests__/hooks/useProducts.test.ts`:
  - [x] **AC-HP-1** Returns `data.products.content` with 1 item after MSW responds
  - [x] **AC-HP-2** `isLoading` is true before MSW responds
  - [x] **AC-HP-3** Changing `filter.query` produces a different `queryKey` (verified via cache keys)

### useProduct tests
- [x] Create `apps/mfe-catalog/src/__tests__/hooks/useProduct.test.ts`:
  - [x] **AC-UPD-1** Returns product data for `id = 'prod-1'`
  - [x] **AC-UPD-2** Returns `null` for an unknown product id
  - [x] **AC-UPD-3** Query is not fired when `id` is `''` (enabled guard)

## 4.19 Acceptance checks

- [x] **AC-1** `npm run dev` from `apps/mfe-catalog/` starts on port 3001 without errors
- [x] **AC-2** `http://localhost:3001` renders the catalog page: search bar, filter pills, store row, product grid
- [x] **AC-3** `http://localhost:3001/remoteEntry.js` is served (Module Federation entry point accessible)
- [x] **AC-4** With API Gateway running, product grid shows real products from `localhost:8080/graphql`
- [x] **AC-5** Typing in the search bar triggers a new GraphQL query after 300 ms; rapid keystrokes collapse to one request
- [x] **AC-6** Selecting a category filter re-queries; grid shows only matching products
- [x] **AC-7** Clicking a product card navigates to `product/:id` under the current mount path and the detail page renders correctly
- [x] **AC-8** "Add to cart" button (on both card and detail page) dispatches `cart:add-item` CustomEvent; verified via integration and component tests
- [x] **AC-9** Wishlist button toggles heart icon fill and `aria-pressed` state
- [x] **AC-10** Product grid shows skeleton cards while the query is in flight
- [x] **AC-11** Pagination controls appear and function correctly when `totalPages > 1`
- [ ] **AC-12** Grid is visually responsive across breakpoints (verify with browser devtools)
- [x] **AC-13** `npm run test` — all test suites pass with zero failures
- [x] **AC-14** `npm run typecheck` — exits with zero TypeScript errors
- [x] **AC-15** `npm run build` produces `dist/remoteEntry.js`
- [x] **AC-16** Styling is implemented with Tailwind utility classes and theme tokens; no component-level CSS files are needed beyond `src/index.css`
